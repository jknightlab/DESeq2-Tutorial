...
<https://swebb1.github.io/RNA-seq_workshop/lesson3_deseq.html>

<https://ashleyschwartz.com/posts/2023/05/deseq2-tutorial>


### Data pre-processing In this practical
We will look at some RNA-Seq data from the Ankylosing Spondylitis Paper
...
To continue, download the data files that we will be using by running the following commands in R.

#### 1. RNA_CD4_raw_counts.txt.gz

Contains the raw counts for RNA expression from CD4 cells from AS patients and HC. They were aligned using STAR and then the reads counted using FeatureCount.

#### 2. patient_information.txt

Has the colData for the patients and healthy controls, the batch information and the sex of the individuals

#### 3. coding_genes_hg19

Links the Ensembl id to gene name, alignments were done using the id's
...
download.file("https://github.com/jknightlab/DESeq2-Tutorial/blob/888f26382cf01a6033ea6398240f51509edc90ce/patient_information.txt", "patient_information.txt")

#To assign gene names to ENSMBL references later we need to download a file that can link the two
download.file("https://github.com/jknightlab/DESeq2-Tutorial/blob/888f26382cf01a6033ea6398240f51509edc90ce/coding_genes_hg19", "coding_genes_hg19")
...
# An alternative approach to filter out low count genes based on the minimum group size
...
Using limma to plot a corrected PCA after running DESeq2 can be useful when you want to account for and remove unwanted variation or batch effects in your data. This process ensures that your PCA results more accurately reflect the biological differences you’re interested in, rather than technical noise or confounding factors.
